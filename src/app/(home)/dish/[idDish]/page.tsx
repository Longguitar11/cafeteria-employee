...
export default function IdDish({ params }: { params: { idDish: string } }) {
  // state
  ...
  const category = categories.find((cate) => cate.id == params.idDish);
  const products = foodAndDrinks.filter((item) => item.idCate == params.idDish);
